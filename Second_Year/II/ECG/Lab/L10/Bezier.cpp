...
vec2 getBezierPoint(std::vector<vec2> controlPoints, float t) {
	// Base case: if there's only one control point, return it
	if (controlPoints.size() == 1) {
		return controlPoints[0];
	}
	std::vector<vec2> nextLevel;
	for (size_t i = 0; i < controlPoints.size() - 1; i++) {
		vec2 point;
		point.x = (1 - t) * controlPoints[i].x + t * controlPoints[i + 1].x;
		point.y = (1 - t) * controlPoints[i].y + t * controlPoints[i + 1].y;
		nextLevel.push_back(point);
	}

	return getBezierPoint(nextLevel, t);
}
